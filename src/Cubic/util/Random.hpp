...
namespace cubic {
    namespace util {
        namespace random {
                ...
                public:
                    ...
                    T generate() {
                        if constexpr (std::is_same_v<T, bool>) {
                            ...
                            return generate<T>(std::numeric_limits<T>::min(), std::numeric_limits<T>::max());
                        } else if constexpr (std::is_floating_point_v<T>) {
                            return generate<T>(0.0, 1.0);
                        } else {
                    ...
                    Out generate(T min_, Y max_) {
                        ...
                        if constexpr (std::is_integral_v<Out>) {
                            ...
                            if constexpr (sizeof(Out) == 1) {
                                return static_cast<Out>(generate<int>(static_cast<int>(min_), static_cast<int>(max_)));
                            } else {
            ...
            inline bool chance(uint8_t probability) {
                return internal::chance(static_cast<double>(probability) / 100.0);
            };
